...
作者：程序员的价投之路
公众号：程序员的价投之路
GitHub：https://github.com/yourusername/bank-stock-analysis
...
class BankStockAnalyzer:
    ...
    def generate_report(self, df, output_file='bank_analysis_report.md'):
        ...
        if 'latest_dividend' in df.columns and not df.empty:
            ...
            report.append(f"2. **最高每股分红**: {top_div['name']} ({top_div['latest_dividend']}元)")
        ...
        if 'dividend_growth_rate' in df.columns and not df.empty:
            ...
            if not df_growth.empty:
                ...
                report.append(f"3. **最高分红增长率**: {top_growth['name']} ({top_growth['dividend_growth_rate']}%)")
